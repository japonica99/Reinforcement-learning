...
from chainer_spiral.dataset import PhotoEnhancementDataset,FaceBeautificationDataset
from chainer_spiral.environments import PhotoEnhancementEnv,FaceBeautificationEnv
...
def main():
    ...
    if config['problem'] == 'photo_enhancement':
    ...
    elif config['problem'] == 'face_beautification':
        def make_env(process_idx, test):
            env = FaceBeautificationEnv(batch_size=config['rollout_n'], max_episode_steps=config['max_episode_steps'],
                             imsize=config['imsize'])
            return env
        ...
        dataset = FaceBeautificationDataset()
