...
from chainer_spiral.environments import PhotoEnhancementEnvDemo,FaceBeautificationEnvDemo
...
def main():
    ...
    if config['problem'] == 'photo_enhancement':
        ...
        def make_env(process_idx, test):
            assert test, "error: test should be True"
            env = FaceBeautificationEnvDemo(batch_size=1,
                                        max_episode_steps=config['max_episode_steps'],
                             imsize=config['imsize'],file_name=args.file_name)
            return env
