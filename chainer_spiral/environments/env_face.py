...
from .edit_face import FaceEditor, edit_demo
import cv2
import random
import logging
import os
...
DATASET_DIR = "./scutfbp5500_dataset/"
TARGET_DIR = "attractive/"
ORIGINAL_DIR = "unattractive/"

class FaceBeautificationEnv(gym.Env):
    action_space = None
    ...
    def __init__(self,
        ...
        self.num_parameters = self.face_editor.num_parameters
    ...
    def reset(self):
        ...
        for i in range(self.batch_size):
            ...
            feature_list = self.face_editor.extract_feature(original_image,(64,64))
            ...
            self.feature_lists.append(feature_list)
    ...
    def step(self, action):
        ...
        for i in range(self.batch_size):
            self.editted_images[i] = self.face_editor(self.original_images[i].copy(), clipped_action[i], self.feature_lists[i]) #landmark不copy 一起变
...
class FaceBeautificationEnvTest(FaceBeautificationEnv):
    ...
    def reset(self):
        ...
        self.original_feature_lists = []

        for i in range(self.batch_size):
            original_image = cv2.imread(self.file_names[i])
            self.original_original_images.append(original_image.copy())
            original_feature_list = self.face_editor.extract_feature(original_image, original_image.shape[:2])
            feature_list = self.face_editor.extract_feature(original_image, (64, 64))
            ...
            self.editted_images.append(editted_image)
            self.feature_lists.append(feature_list)
            self.original_feature_lists.append(original_feature_list)
    ...
    def step(self, action):
        ...
        for i in range(self.batch_size):
            self.editted_images[i] = self.face_editor(self.original_images[i].copy(), clipped_action[i], self.feature_lists[i]) #landmark不copy 一起变
            self.original_editted_images.append(self.face_editor(self.original_original_images[i].copy(), clipped_action[i],self.original_feature_lists[i]))
...
class FaceBeautificationEnvDemo(FaceBeautificationEnvTest):
    ...
    def __init__(self,
        ...
        super(FaceBeautificationEnvTest, self).__init__(batch_size=batch_size,
                         logger=logger,
                         imsize=imsize,
                         max_episode_steps=max_episode_steps)
    ...
    def step(self, action):
        ...
        if done:
            for i in range(self.batch_size):
                edit_demo(self.original_original_images[i], clipped_action[i])
